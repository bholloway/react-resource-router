...
describe('resource store', () => {
  ...
  const describeFeatureFlag = (name: string, value: boolean, fn: () => void) =>
    describe(`${name}=${value}`, () => {
      beforeEach(() => {
        // @ts-ignore
  ...
  describe('actions', () => {
    ...
    describe('getResourceFromRemote', () => {
      ...
      describe('where getData() synchronously returns previous data', () => {
        describeFeatureFlag(
          'endeavour.spa.route-resource-consistency',
          true,
          () => {
      ...
      describe('where getData() synchronously returns new data', () => {
        describeFeatureFlag(
          ...
          () => {
            it.each([false, true])(
              'should enter loading state with new data as promise [BUG] (prefetch=%s)',
              async prefetch => {
      ...
      describe('where getData() times out', () => {
        describeFeatureFlag(
          ...
          () => {
            it.each([false, true])(
              'should enter loading state with promise unrepresentive of rejecting TimeoutError [BUG] and clear existing data on loaded [BUG] (prefetch=%s, w/ timeout)',
              async prefetch => {
                const getData = jest
                  .fn()
                ...
                await expect(slice1.promise).resolves.toEqual('data2'); // BUG!
                const returnValue = await pending;
  ...
  describe('consistency of promise and data under race conditions', () => {
    ...
    describe('when update occurs outside loading', () => {
      describeFeatureFlag(
        ...
        false,
        () => {
          it('should retain stale error [BUG] and stale promise [BUG] on update', async () => {
          ...
          it('should set promise when loading but keep data and error (assume not expired)', async () => {
            actions.updateResourceState(
              mockResource,
              mockRouterStoreContext,
              () => 'data2'
          ...
          it('should clear error and set new data and on loaded', async () => {
            actions.updateResourceState(
              mockResource,
              mockRouterStoreContext,
              () => 'data2'
    ...
    describe('when update occurs during loading', () => {
      describeFeatureFlag(
        ...
        false,
        () => {
          it('should set promise when loading but retain data and error (assume not expired)', async () => {
            actions.getResourceFromRemote(
              mockResource,
              mockRouterStoreContext,
              mockOptions
          ...
          it('should retain stale error [BUG] and async promise on update', async () => {
            actions.getResourceFromRemote(
              mockResource,
              mockRouterStoreContext,
              mockOptions
          ...
          it('should clear error and set new data on loaded', async () => {
            const pending = actions.getResourceFromRemote(
              mockResource,
              mockRouterStoreContext,
              mockOptions
          ...
          it('should reinstate stale initial data [BUG] and set error on failed', async () => {
    ...
    describe('when clear occurs during loading', () => {
      describeFeatureFlag(
        ...
        () => {
          ...
          it('should not reinstate resource slice on failed, should reinstate stale data [BUG]', async () => {
            const rejection = new Error('failing');
            const pending = actions.getResourceFromRemote(
              { ...mockResource, getData: () => Promise.reject(rejection) },
              mockRouterStoreContext,
